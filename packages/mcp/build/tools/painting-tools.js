...
function getImageFamily(apiModel) {
    if (apiModel.startsWith('gemini-'))
        return 'gemini-native';
    if (apiModel.startsWith('openai/'))
        return 'openai';
    if (apiModel.startsWith('google/imagen-'))
        return 'imagen';
    if (apiModel.startsWith('bailian/'))
        return 'bailian';
    if (apiModel.startsWith('qianfan/qwen-'))
        return 'qwen';
    if (apiModel.startsWith('doubao/'))
        return 'doubao';
    if (apiModel.startsWith('bfl/') || apiModel.startsWith('FLUX'))
        return 'flux';
    if (apiModel.startsWith('ideogram/'))
        return 'ideogram';
    return 'generic';
}
 ...
 */
function buildPredictionsInput(args, apiModel) {
    ...
    switch (family) {
        case 'openai':
            ...
            if (args.moderation && !ref)
                input.moderation = args.moderation;
            if (args.background)
                input.background = args.background;
            if (args.output_format)
                input.output_format = args.output_format;
            if (args.input_fidelity && ref)
                input.input_fidelity = args.input_fidelity;
            if (ref)
                input.image = ref;
            return input;
        case 'imagen':
            if (args.n)
                input.sampleCount = args.n;
            if (args.aspect_ratio)
                input.aspectRatio = args.aspect_ratio;
            return input;
        case 'qwen':
            if (size)
                input.size = size;
        ...
        case 'doubao':
            ...
            if (args.response_format)
                input.response_format = args.response_format;
            if (ref)
                input.image = ref;
            return input;
        case 'flux':
            ...
            if (typeof args.seed === 'number')
                input.seed = args.seed;
            if (ref)
                input.input_image = ref;
            return input;
        case 'ideogram':
            if (args.rendering_speed)
                input.rendering_speed = args.rendering_speed;
            if (args.aspect_ratio)
                input.aspect_ratio = args.aspect_ratio;
            return input;
        case 'bailian': {
            ...
            if (typeof args.watermark === 'boolean')
                input.watermark = args.watermark;
            if (typeof args.thinking_mode === 'boolean')
                input.thinking_mode = args.thinking_mode;
            if (typeof args.prompt_extend === 'boolean')
                input.prompt_extend = args.prompt_extend;
...
function extractImageContents(data) {
    const contents = [];
    const seen = new Set();
    const addImage = (b64, mimeType = 'image/png') => {
        if (b64 && !seen.has(b64)) {
            seen.add(b64);
    ...
    if (Array.isArray(out)) {
        for (const item of out) {
            if (isHttpUrl(item))
                ...
                addUrl(item.url);
            else if (typeof item?.bytesBase64 === 'string')
                addImage(item.bytesBase64);
        }
    }
    if (Array.isArray(out?.b64_json)) {
        for (const item of out.b64_json) {
            if (typeof item?.bytesBase64 === 'string')
                addImage(item.bytesBase64);
        }
    }
    ...
    if (isHttpUrl(data?.url))
        addUrl(data.url);
    if (isHttpUrl(data?.image_url))
        addUrl(data.image_url);
...
async function performJsonRequest(apiKey, url, body) {
    const response = await fetch(url, {
        method: 'POST',
...
async function createGeminiNativeImage(apiKey, apiModel, args) {
...
export const paintingTools = {
    image_generate: {
        description: 'Generate images with AIHubMix. Models: nano-banana-pro / nano-banana-2 (Google Gemini native, aspect_ratio + size 1K/2K/4K, google_search); ' +
        ...
        async execute(args) {
            ...
            if (!def) {
                throw new McpError(ErrorCode.InvalidParams, `Unknown image model: ${modelKey}. Available: ${Object.keys(IMAGE_MODELS).join(', ')}`);
            }
            ...
            try {
                const data = family === 'gemini-native'
                ...
                if (contents.length === 0) {
                    ...
                    const pollingUrl = Array.isArray(out)
                        ? out.find((item) => item?.polling_url)?.polling_url
                        : out?.polling_url;
                    if (pollingUrl) {
                        try {
    ...
    video_generate: {
        description: 'Generate videos with AIHubMix (async: submits a task then polls until ready). Models: ' +
        ...
        async execute(args) {
            ...
            if (!registryId) {
                throw new McpError(ErrorCode.InvalidParams, `Unknown video model: ${modelKey}. Available: ${Object.keys(VIDEO_MODELS).join(', ')}`);
            }
            if (!args.prompt || typeof args.prompt !== 'string' || args.prompt.trim().length === 0) {
                throw new McpError(ErrorCode.InvalidParams, 'Prompt is required and must be a non-empty string');
            }
            ...
            if (typeof args.input_reference === 'string' && args.input_reference.trim()) {
                if (!cap.caps.includes('i2v') && !cap.apiModelI2V) {
                    throw new McpError(ErrorCode.InvalidParams, `${cap.label || modelKey} does not support input_reference (image-to-video)`);
                }
                const inline = await urlToInlineData(args.input_reference.trim());
            ...
            try {
                ...
                if (!submitResponse.ok) {
                    ...
                    throw new McpError(ErrorCode.InternalError, `Video request failed with status ${submitResponse.status}: ${errorText}`);
                }
                ...
                if (completed) {
                    ...
                    lines.push(`Still processing after ${Math.round(timeoutMs / 1000)}s. Check status: GET ${apiBase}/videos/${taskId}`);
                }
